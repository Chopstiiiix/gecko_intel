...
/**
 * GECKO — Internet Outage Detection (IODA)
...
export async function GET() {
  try {
    ...
    const res = await fetch(url, {
      ...
      cache: 'no-store',
      headers: { 'User-Agent': 'GECKO/4.2', 'Accept': 'application/json' },
    });

    console.log('[GECKO] IODA response status:', res.status);
    ...
    console.error('[GECKO] IODA fetch error:', error);
