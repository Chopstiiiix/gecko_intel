...
/**
 * GECKO — Active Fire & Wildfire Tracking
...
export async function GET() {
  try {
    ...
    for (const url of firmsSources) {
      try {
        const res = await fetch(url, {
          ...
          headers: { 'User-Agent': 'GECKO-Intelligence-Platform/3.5' },
        });
        if (res.ok) {
    ...
    } catch (e) { console.warn('[GECKO] Suppressed EONET error:', e instanceof Error ? e.message : e); }
