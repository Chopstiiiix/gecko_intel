...
const GeckoMap = dynamic(() => import('@/components/GeckoMap'), { ssr: false });
const LayerPanel = dynamic(() => import('@/components/LayerPanel'));
const CameraViewer = dynamic(() => import('@/components/CameraViewer'));
const OsintPanel = dynamic(() => import('@/components/OsintPanel'));
const EntityGraphPanel = dynamic(() => import('@/components/EntityGraphPanel'));
const TokenPanel = dynamic(() => import('@/components/TokenPanel'));
const DataEntryPanel = dynamic(() => import('@/components/DataEntryPanel'));
...
export default function Dashboard() {
  ...
  const [geckoTheme, setGeckoTheme] = useState<'core'|'ghost'>('ghost');

  useEffect(() => {
    document.body.className = geckoTheme === 'core' ? '' : `theme-${geckoTheme}`;
  }, [geckoTheme]);
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      ...
      if (e.key === 'n') setShowDataEntry(p => !p);
  ...
  const handleRightClick = useCallback(async (coords: { lat: number; lng: number }) => {
    ...
    } catch (e) { console.warn('[GECKO] Suppressed error:', e instanceof Error ? e.message : e); } finally { setDossierLoading(false); }
  }, []);
  ...
  useEffect(() => {
    (window as any).openGeckoIntel = (entity: any) => {
    ...
    return () => { delete (window as any).openGeckoIntel; };
  }, []);
  ...
  const fetchEndpoint = useCallback(async (url: string, transform?: (d: any) => any, options?: RequestInit) => {
    ...
    try {
      ...
      console.warn('[GECKO] Suppressed error:', e instanceof Error ? e.message : e);
      setBackendStatus('error');
    }
  }, []);

  // ── NIGERIA INTEL (Gecko) — operator-entered + seed data ──
  const refetchNigeria = useCallback(() => {
    fetchEndpoint('/api/nigeria/incidents', d => ({ ng_incidents: d.items }));
    fetchEndpoint('/api/nigeria/infrastructure', d => ({ ng_infrastructure: d.items }));
    fetchEndpoint('/api/nigeria/checkpoints', d => ({ ng_checkpoints: d.items }));
    fetchEndpoint('/api/nigeria/roads', d => ({ ng_roads: d.items }));
  }, [fetchEndpoint]);

  useEffect(() => {
    refetchNigeria();
    const iv = setInterval(refetchNigeria, 120000); // refresh field intel every 2 min
    return () => clearInterval(iv);
  }, [refetchNigeria]);

  // Nigeria news feed (geoparsed headlines) — heavier, fetched only when layer is on
  useEffect(() => {
    if (!activeLayers.ng_news) return;
    const load = () => fetchEndpoint('/api/nigeria-news', d => ({ ng_news_items: d.items }));
    load();
    const iv = setInterval(load, 600000); // 10 min
    return () => clearInterval(iv);
  }, [activeLayers.ng_news, fetchEndpoint]);
  ...
  // ── GECKO SDK — Intelligence Fusion Layer ──
  ...
  return (
    <main className="fixed inset-0 w-full h-full bg-[var(--bg-void)] overflow-hidden">
      ...
      <AnimatePresence>
        {showSplash && (
          ...
          >
            ...
            {/* ── GECKO title — letter-by-letter stagger ── */}
            <div className="flex items-center gap-[2px] mb-3 z-[2]">
              {'GECKO'.split('').map((letter, i) => (
                <motion.span
                  key={i}
      ...
      <AnimatePresence>
        {showDataEntry && (
          <DataEntryPanel
            ...
            onSaved={refetchNigeria}
            getCursorCoord={() => mouseCoordsRef.current}
      ...
      <button
        ...
        className="absolute top-[70px] right-3 z-[350] glass-panel px-3 py-2 pointer-events-auto flex items-center gap-2 hover:border-[var(--gold-primary)]/50 transition-colors group"
        style={{ display: showDataEntry ? 'none' : 'flex' }}
        title="Add Nigeria field intel (N)"
      >
        <Crosshair className="w-4 h-4 text-[var(--gold-primary)] group-hover:scale-110 transition-transform" />
        <span className="hidden md:inline text-[10px] font-mono tracking-widest text-[var(--text-primary)]">DATA ENTRY</span>
      </button>
      ...
      <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1, delay: 2.5 }} className={`absolute top-4 z-[200] pointer-events-none flex flex-col`} style={{ left: isMobile ? '24px' : '96px', right: '24px' }}>
        <div className="flex items-center gap-3 w-fit">
          ...
          <div className="flex flex-col items-start gap-0.5">
            <h1 className="text-lg md:text-xl font-bold tracking-[0.4em] text-[#D4AF37] font-mono">GECKO</h1>
        ...
        <div className="flex items-center gap-3 mt-1.5 pl-[44px] min-w-0 pr-4">
          <span className="text-[5px] md:text-[6px] text-[var(--text-muted)] font-mono tracking-[0.2em] md:tracking-[0.3em] uppercase opacity-40 truncate">
            POWERED BY GECKO OPEN SOURCE INTELLIGENCE <span className="hidden md:inline">· C2 ENGINE: PHYSICAL COMMAND CORE · SENSORS: ORBITAL LATTICE · NET: LYCAN NETWORK</span>
          </span>
        </div>
      </motion.div>
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 3 }} className="status-bar-desktop absolute top-4 right-6 z-[200] pointer-events-none flex items-center gap-4 text-[9px] font-mono tracking-widest text-[var(--text-muted)]">
        ...
        <a href='https://ko-fi.com/M8D41ZYW4Z' target='_blank' rel='noopener noreferrer' className="pointer-events-auto glass-panel px-3 py-1.5 flex items-center gap-1.5 text-[8px] font-mono tracking-widest hover:opacity-80 transition-opacity border-[var(--gold-primary)]/40 bg-[var(--gold-primary)]/10 ml-4 shadow-[0_0_10px_rgba(255,215,0,0.1)]">
          <div className="w-1.5 h-1.5 rounded-full bg-[var(--gold-primary)] animate-gecko-pulse" />
      ...
      {isMobile && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 2.5 }} className="absolute top-3 right-3 z-[200] pointer-events-auto flex items-center gap-2">
          ...
          <a href='https://ko-fi.com/M8D41ZYW4Z' target='_blank' rel='noopener noreferrer' className="glass-panel px-2 py-1 flex items-center gap-1.5 text-[7px] font-mono tracking-widest hover:opacity-80 transition-opacity border-[var(--gold-primary)]/40 bg-[var(--gold-primary)]/10">
            <div className="w-1 h-1 rounded-full bg-[var(--gold-primary)] animate-gecko-pulse" />
            <span className="text-[var(--gold-primary)] font-bold">SUPPORT PROJECT</span>
          </a>
        </motion.div>
      )}
                ...
                <div className="px-3 pb-3">
                  <div className="flex items-center justify-between mb-2">
                    <span className="hud-text text-[9px] text-[var(--text-primary)]">
                      {mobilePanel === 'layers' ? 'LAYERS & STATS' : mobilePanel === 'markets' ? 'MARKETS & INTEL' : mobilePanel === 'intel' ? 'INTEL FEED' : mobilePanel === 'recon' ? 'GECKO RECON' : 'SEARCH'}
                    </span>
                  ...
                  {mobilePanel === 'layers' && (
                    <>
                      ...
                      <LayerPanel data={data} activeLayers={activeLayers} setActiveLayers={setActiveLayers} isMobile={true} theme={geckoTheme} setTheme={setGeckoTheme} />
                      <div className="mt-8">
      ...
      {(regionDossier || dossierLoading) && (
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="absolute top-16 md:top-20 left-2 right-2 md:left-1/2 md:right-auto md:-translate-x-1/2 z-[300] md:w-[480px] max-h-[65vh] overflow-y-auto styled-scrollbar">
          <div className="glass-panel p-5 gecko-glow">
