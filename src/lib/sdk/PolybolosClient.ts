 ...
 * ║                                                                 ║
 * ║  Subscribes to GECKO feeds + external providers (Lattice),     ║
...
function translateFlights(flights: any[], subtype: string): PolybolosEntity[] {
  ...
  return flights.map((f: any) => ({
    id: `gecko-air-${f.icao24 || f.callsign || Math.random().toString(36).slice(2)}`,
    ...
    source: { provider: 'gecko', feed: `flights-${subtype}`, originalId: f.icao24, confidence: 0.9 },
...
function translateMaritime(ships: any[]): PolybolosEntity[] {
  ...
  return ships.map((s: any) => ({
    id: `gecko-sea-${s.mmsi || s.id || Math.random().toString(36).slice(2)}`,
    ...
    classification: Classification.UNCLASSIFIED,
    source: { provider: 'gecko', feed: 'maritime-ais', originalId: s.mmsi?.toString(), confidence: 0.85 },
    timestamp: new Date().toISOString(),
...
function translateEarthquakes(events: any[]): PolybolosEntity[] {
  ...
  return events.map((eq: any) => ({
    id: `gecko-event-eq-${eq.id || Math.random().toString(36).slice(2)}`,
    ...
    classification: Classification.UNCLASSIFIED,
    source: { provider: 'gecko', feed: 'usgs-earthquakes', originalId: eq.id, confidence: 0.99 },
    timestamp: new Date().toISOString(),
...
function translateSatellites(sats: any[]): PolybolosEntity[] {
  ...
  return sats.map((s: any) => ({
    id: `gecko-space-${s.noradId || Math.random().toString(36).slice(2)}`,
    ...
    classification: Classification.UNCLASSIFIED,
    source: { provider: 'gecko', feed: 'satnogs', originalId: s.noradId?.toString(), confidence: 0.95 },
    timestamp: new Date().toISOString(),
...
function translateFires(fires: any[]): PolybolosEntity[] {
  ...
  return fires.map((f: any, i: number) => ({
    ...
    classification: Classification.UNCLASSIFIED,
    source: { provider: 'gecko', feed: 'nasa-firms', confidence: 0.9 },
    timestamp: new Date().toISOString(),
...
function translateCCTV(cameras: any[]): PolybolosEntity[] {
  ...
  return cameras.map((c: any) => ({
    id: `gecko-sensor-cctv-${c.id || Math.random().toString(36).slice(2)}`,
    name: c.name || 'Camera',
    domain: Domain.LAND,
    ...
    threat: ThreatLevel.NONE,
    classification: Classification.UNCLASSIFIED,
    source: { provider: 'gecko', feed: 'cctv-network', originalId: c.id, confidence: 1.0 },
    timestamp: new Date().toISOString(),
...
function translateRadiation(stations: any[]): PolybolosEntity[] {
  ...
  return stations.map((r: any) => ({
    id: `gecko-sensor-rad-${r.name || Math.random().toString(36).slice(2)}`,
    ...
    classification: Classification.UNCLASSIFIED,
    source: { provider: 'gecko', feed: 'radiation-network', confidence: 0.95 },
    timestamp: new Date().toISOString(),
...
export class PolybolosClient {
  ...
  /** Connect to the GECKO SSE stream endpoint */
  private connectSSE(): void {
    if (typeof EventSource === 'undefined') return;

    try {
      this.sseConnection = new EventSource(
        `${this.config.geckoBaseUrl}/api/sdk/stream`
      );
  ...
  /**
   * Ingest raw GECKO data and translate it into Polybolos entities.
   * This is the primary method called by page.tsx to feed data into the SDK.
   */
  ingestGeckoData(data: Record<string, any>): void {
  ...
  private getActiveFeedCount(): number {
    let count = 0;
    // Count GECKO feeds that have data
