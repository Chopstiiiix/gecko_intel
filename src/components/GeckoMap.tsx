...
function GeckoMap({ data, activeLayers, onEntityClick, onMouseCoords, onRightClick, onViewStateChange, flyToLocation, projection = 'globe', mapStyle = 'dark', sweepData, scanTargets = [], demoMode = false, theme = 'core' }: GeckoMapProps) {
  ...
  useEffect(() => {
    ...
    const map = new maplibregl.Map({
      ...
      style: styleUrl,
      center: [8.6753, 9.0820], zoom: 5.6, minZoom: 1.5, maxZoom: 18,
    ...
    map.on('load', () => {
      ...
      map.addLayer({ id: 'ng-incidents-glow', type: 'circle', source: 'ng-incidents', paint: {
        'circle-radius': ['interpolate', ['linear'], ['zoom'], 4, 10, 8, 18, 12, 28], 'circle-color': sevColor, 'circle-opacity': 0.16, 'circle-blur': 1 }});
      map.addLayer({ id: 'ng-incidents-dots', type: 'circle', source: 'ng-incidents', paint: {
        'circle-radius': ['interpolate', ['linear'], ['zoom'], 4, 5, 8, 7, 12, 11], 'circle-color': sevColor,
        'circle-stroke-width': 1.5, 'circle-stroke-color': '#0A0A0A', 'circle-opacity': 0.95 }});
      map.addLayer({ id: 'ng-incidents-label', type: 'symbol', source: 'ng-incidents', minzoom: 7, layout: {
        'text-field': ['get', 'label'], 'text-size': 9, 'text-font': ['Open Sans Bold'], 'text-offset': [0, 1.3], 'text-anchor': 'top', 'text-optional': true,
      }, paint: { 'text-color': '#FFB3A7', 'text-halo-color': '#000', 'text-halo-width': 1.3, 'text-opacity': 0.85 }});
    ...
    map.on('click', 'ng-news-dots', (e: any) => {
      if (!e.features?.length) return;
      const p = e.features[0].properties as any;
      const coords = (e.features[0].geometry as any).coordinates;
      const color = '#E9E5D6';
  ...
  // ── NIGERIA INTEL → GeoJSON (Gecko) ──
  useEffect(() => {
    if (!mapReady) return;
    const ptFeatures = (arr: any[]) => (arr || [])
    ...
    setGeo('ng-incidents', activeLayers.ng_incidents ? ptFeatures(data.ng_incidents) : []);
    setGeo('ng-infra', activeLayers.ng_infrastructure ? ptFeatures(data.ng_infrastructure) : []);
    setGeo('ng-checkpoints', activeLayers.ng_checkpoints ? ptFeatures(data.ng_checkpoints) : []);
    // roads are already GeoJSON LineString features from the API
    setGeo('ng-roads', activeLayers.ng_roads ? (data.ng_roads || []).filter((f: any) => f?.geometry?.coordinates?.length >= 2) : []);
    setGeo('ng-news', activeLayers.ng_news ? ptFeatures(data.ng_news_items) : []);
  }, [mapReady, setGeo, data.ng_incidents, data.ng_infrastructure, data.ng_checkpoints, data.ng_roads, data.ng_news_items, activeLayers.ng_incidents, activeLayers.ng_infrastructure, activeLayers.ng_checkpoints, activeLayers.ng_roads, activeLayers.ng_news]);
  ...
  useEffect(() => {
    ...
    try {
      if (enabled) {
        ...
        if (!map.getSource('gecko-buildings')) {
          map.addSource('gecko-buildings', {
            type: 'vector',
        ...
        if (!map.getLayer('gecko-3d-buildings')) {
          map.addLayer({
            id: 'gecko-3d-buildings',
            source: 'gecko-buildings',
        ...
        // ── DISABLE 3D ──
        if (map.getLayer('gecko-3d-buildings')) map.removeLayer('gecko-3d-buildings');
      }
    } catch (e) {
      console.warn('[GECKO] 3D terrain toggle error:', e);
    }
