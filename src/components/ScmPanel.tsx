...
export default function ScmPanel({ data }: ScmPanelProps) {
  ...
  return (
    <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.7, duration: 0.6 }} className="glass-panel p-3 pointer-events-auto mt-3 border border-[#5F8443]/30" style={{ background: 'rgba(0, 188, 212, 0.05)' }}>
      <button onClick={() => setExpanded(!expanded)} className="flex items-center justify-between w-full mb-2">
        <div className="flex items-center gap-2">
          <Target className="w-3.5 h-3.5 text-[#5F8443]" />
        ...
        {expanded && (
          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.2 }} className="overflow-hidden flex flex-col flex-1 min-h-0">
            <div className="space-y-3 mt-2 overflow-y-auto styled-scrollbar pr-1 flex-1 pb-4">
              ...
              {marketAlerts.length > 0 && (
                <div>
                  <div className="flex items-center gap-1.5 mb-1.5">
                    <AlertCircle className="w-3 h-3 text-[#D29B3B]" />
                    <span className="text-[9px] font-mono text-[#D29B3B] tracking-widest font-bold">MARKET IMPACT ALERTS</span>
                  </div>
              ...
              <div>
                <div className="flex items-center gap-1.5 mb-1.5">
                  <AlertTriangle className="w-3 h-3 text-[#B0492F]" />
                  <span className="text-[9px] font-mono text-[var(--text-muted)] tracking-widest">CRITICAL SUPPLIERS</span>
                </div>
                {criticalSuppliers.length === 0 ? (
                  <div className="text-[9px] font-mono text-[#5F8443] px-2">✓ All monitored Tier 1/2 nodes operational.</div>
                ) : (
                  <div className="space-y-1">
                    {criticalSuppliers.map((s: any, i: number) => {
                      const threats = s.active_threats ? JSON.parse(s.active_threats) : [];
                      return (
                        <div key={i} className="px-2 py-1.5 rounded border border-[#B0492F]/40 bg-[#B0492F]/10">
                          <div className="flex justify-between items-start mb-1">
                            <span className="text-[10px] font-mono font-bold text-[#B0492F]">{s.name}</span>
                            <span className="text-[8px] font-mono text-[#B0492F]/80">{s.city}</span>
                          </div>
                          <div className="text-[8px] font-mono text-[#E9E5D6]">{threats.join(' • ')}</div>
                        </div>
                      );
                    })}
                  </div>
              ...
              <div>
                <div className="flex items-center gap-1.5 mb-1.5">
                  <Anchor className="w-3 h-3 text-[#D29B3B]" />
                ...
                {(congestedPorts.length === 0 && riskyChokes.length === 0) ? (
                  <div className="text-[9px] font-mono text-[#5F8443] px-2">✓ Global maritime flow optimal.</div>
                ) : (
                  <div className="space-y-1">
                    {riskyChokes.map((c: any, i: number) => (
                      <div key={`c-${i}`} className="px-2 py-1.5 rounded hover:bg-white/5 transition-colors border-l-2" style={{ borderLeftColor: c.risk === 'CRITICAL' ? '#B0492F' : '#D29B3B' }}>
                        <div className="flex justify-between items-center mb-0.5">
                          <span className="text-[10px] font-mono text-[#D29B3B] font-bold">{c.name}</span>
                          <span className="text-[8px] font-mono font-bold px-1 rounded" style={{ background: c.risk === 'CRITICAL' ? '#B0492F' : '#D29B3B', color: '#000' }}>{c.risk}</span>
                        </div>
                        <div className="text-[8px] font-mono text-[#aaa]">{c.traffic}</div>
                      </div>
                    ))}
                    {congestedPorts.map((p: any, i: number) => (
                      <div key={`p-${i}`} className="px-2 py-1.5 rounded hover:bg-white/5 transition-colors border-l-2" style={{ borderLeftColor: p.congestion === 'SEVERE' ? '#B0492F' : '#D29B3B' }}>
                        <div className="flex justify-between items-center mb-0.5">
                          <span className="text-[10px] font-mono text-[#5F8443]">{p.name}</span>
                          <span className="text-[8px] font-mono font-bold px-1 rounded" style={{ background: p.congestion === 'SEVERE' ? '#B0492F' : '#D29B3B', color: '#000' }}>{p.congestion}</span>
                        </div>
