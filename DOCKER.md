# Self-Hosting GECKO with Docker

GECKO ships as a self-contained Next.js standalone build. This guide covers
running it with Docker / Docker Compose, deploying it as a [CasaOS](https://casaos.io)
app, and configuring the optional API keys.

> **TL;DR:** GECKO runs fully **without any API keys**. All core feeds
...
- **`env_file: .env` (`required: false`)** — if a `.env` file exists its
  values are injected into the container; if it's missing, GECKO still starts
  with the keyless feeds.
- **`ports: ${GECKO_PORT:-3000}:3000`** — the web UI. The container always
  listens on 3000; the published **host** port is `GECKO_PORT` (default
  `3000`). Set `GECKO_PORT` in `.env` to remap it, e.g. `GECKO_PORT=3005`
...
Registry, so you can run GECKO without building anything:

```bash
docker pull <your-registry>/gecko-intel:latest      # or a pinned tag, e.g. :0.1.0
docker run -d --name gecko \
  -p 3005:3000 --env-file .env --restart unless-stopped \
  <your-registry>/gecko-intel:latest
```
...
```bash
docker build -t gecko:latest .
docker run -d --name gecko -p 3000:3000 --env-file .env --restart unless-stopped gecko:latest
```
...
3. GECKO appears on the dashboard with its icon, reachable on host port
   `3000` (or whatever `GECKO_PORT` you set in `.env`).
...
> directly, either build/tag `gecko:latest` first
> (`docker build -t gecko:latest /path/to/gecko`) or set `image:` to a
...
| `SCANNER_KEY` | Shared secret; **must equal the backend's `GECKO_KEY`** | RECON toolkit |

Without `SCANNER_URL`/`SCANNER_KEY` the RECON endpoints return `503` and the
rest of GECKO works normally. Generate a key with `openssl rand -hex 32`.
...
|----------|---------|---------|
| `GECKO_TELEGRAM_CHANNELS` | Comma-separated list of public Telegram channel usernames (no `@`) to scrape for the **Telegram OSINT** map layer. Overrides the curated default set. | `osintdefender,insiderpaper,aljazeeraenglish,nexta_live,war_monitor` |
| `GECKO_PORT` | Host port the compose file publishes (container itself always listens on 3000). | `3000` |
